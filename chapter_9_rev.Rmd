...
lmerTest是一个与lme4包兼容的包，主要用于对混合效应模型进行假设检验；其中也包含了lmerTest::lmer()函数，与lme4::lmer()不同的是，其结果报告了显著性（使用Satterthwaite自由度）
...
```{r}
## 使用lmerTest建立的模型，基于Satterthwaite自由度的的F检验。
lmer_model %>% anova()
```
...
model_aov1 =  mt_raw %>%
  ...
  lmerTest::lmer(
   data = .,
   RT ~ Identity * Valence + (1|Sub) + (1|Identity:Sub) + (1|Valence:Sub) + (1|Sub:Identity:Valence))
